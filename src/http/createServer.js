  ...
  return httpCreateServer(async (req, res) => {
    ...
    try {
      ...
      if (path.startsWith("/api/")) {
        if (path === "/api/integrations/piper/intake" || path === "/api/integrations/deal-findr/intake") {
          if (req.method !== "POST") {
          ...
          if (path !== "/api/v1/system/status" && path !== "/api/v1/opportunities/convert") {
            if (ctx.config.env === "production" && ctx.config.integrationEnabled) {
              ...
              if (!isS2S) {
                ...
                if (req.method === "POST" && path.startsWith("/api/v1/piper/")) {
                  ...
                  if (path === "/api/v1/piper/run" && !session.permissions.includes("pipeline.manage")) {
                    return sendJson(res, 403, { ok: false, error: "forbidden_insufficient_permissions" });
                  }
                }
              } else if (req.method === "POST" && path === "/api/v1/piper/run") {
                const canManage = s2sIdentity.scope.includes("ocg-one.pipeline.manage") ||
                  s2sIdentity.permissions.includes("pipeline.manage") ||
                  s2sIdentity.permissions.includes("ocg-one.pipeline.manage");
                if (!canManage) return sendJson(res, 403, { ok: false, error: "forbidden_insufficient_permissions" });
              }
            }
          }
